export interface Args {
  cmd?: 'trace' | 'snap' | 'diff' | 'hook' | 'hooks' | 'tools';
...
export function parseArgs(argv: string[], now = Date.now()): Args {
  const a: Args = { since: now - 7 * 86_400_000, sinceExplicit: false, top: 5, ctxLimit: 150_000, follow: false, json: false, md: false, brief: false, cost: false, poolpool: false, hooksInstall: false, hooksPrint: false, hooksGlobal: false, hooksSuggest: false, hooksList: false, hooksKeepLegacy: false, help: false, version: false };
  if (argv[0] === 'trace') {
    a.cmd = 'trace';
    argv = argv.slice(1);
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (x === '--since' || x === '-s') {
      ...
      const v = next();
      if (v !== 'session' && v !== 'day') throw new Error(`--by needs session or day, got: ${v}`);
      a.by = v;
    }
...
usage: tally [--since 7d] [--project <slug-substr>] [--top 5] [--ctx-limit 150k] [--json]
       ...
       tally hooks --install [--global] [--keep-legacy] | --print | --list | --suggest   wire tally's hooks into .claude/settings.json (--install absorbs the hand-written curl|sh + sed-guard.sh hooks pre-bash covers unless --keep-legacy, and runs squirt init --claude when squirt is on PATH; --list: show every configured hook + who owns it; --suggest: print-only, top error heads → a guard to consider)
  ...
  --by        session | day — session: full heaviest-sessions table, no top cap; day: totals per UTC date
