...
import { BIG_RESULT_CHARS, DEFAULT_CTX_LIMIT, bashLooksLikeLogDump } from './analyze.js';
import { cap, fmt, pad } from './render.js';
import { escapeRegExp, estTokens, isWithinDir, usedTools } from './parse.js';
...
/** last MAIN-LOOP `assistant` record's cache-read tokens — same "context" convention as
...
export function lastUsage(transcriptPath: string): { cacheRead: number } | null {
  let fd: number;
  try {
  ...
  try {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
      ...
      if (o.type === 'assistant' && !o.isSidechain && o.message?.usage) {
        return { cacheRead: o.message.usage.cache_read_input_tokens ?? 0 };
      }
    }
    return null;
  } finally {
...
function runCtxGuard(input: HookInput, env: NodeJS.ProcessEnv): HookResult {
  const limit = Number(env.TALLY_CTX_LIMIT) || DEFAULT_CTX_LIMIT;
  ...
  saveCtxState(statePath, state);
  const prefix = is2x ? '⚠⚠ ' : '';
  const additionalContext = `${prefix}tally: ctx ~${fmt(usage.cacheRead)} (limit ${fmt(limit)}) — finish the current step, then hand the remaining work to a subagent (Agent tool) or tell the user to /compact / start a new session; do not read more files into this context`;
  return {
    exit: 0,
...
function looksLikeCurlPipeShell(command: string): boolean {
  return HAS_CURL_WGET_RE.test(command) && PIPES_TO_SHELL_RE.test(command);
}
...
function runPostTool(input: HookInput): HookResult {
  ...
  const additionalContext = `tally: last result ~${fmt(estTokens(size))} tok — trim next time (squirt / head / Read limit)`;
  return {
    exit: 0,
...
function runPostBashMark(input: HookInput, env: NodeJS.ProcessEnv): HookResult {
  ...
  // command-position match only (parse.ts usedTools) — `pulse --brief` / `cd ~/git/brief` are not usage
  const matched = usedTools(command, feedbackTools(env)).filter(
...
function runStopFeedback(input: HookInput, env: NodeJS.ProcessEnv): HookResult {
  ...
  const message = untouched.length === 1
    ? `FEEDBACK.md not updated for: ${untouched[0]} — append a dated section to ${join(home, untouched[0], 'FEEDBACK.md')}`
    : `FEEDBACK.md not updated for: ${untouched.join(', ')} — append a dated section to each ~/git/<tool>/FEEDBACK.md`;
  return { exit: 2, stdout: '', message };
}
...
export async function cmdHooks(opts: HooksCmdOpts, env: NodeJS.ProcessEnv = process.env): Promise<HooksCmdResult> {
  ...
  const hooksDir = opts.hooksDir ?? join(root, '.claude', 'hooks');
