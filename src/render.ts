...
import type { GuardRow, SkillRow, ToolUsageRow } from './tools.js';
...
export function renderText(r: Report, opts: { top: number; by?: 'session' | 'day'; poolpool?: PoolpoolRow[]; cost?: boolean }): string {
  const L: string[] = [];
  L.push(`tally — ${windowLabel(r)} window · ${r.files} transcripts · ${r.sessions} sessions · ${r.turns.toLocaleString()} API turns`);
  ...
  if (opts.by === 'day') {
    ...
    L.push('by project' + ' '.repeat(18) + 'sess  turns   out    cache-w  tool-out');
    for (const p of r.byProject.slice(0, opts.top * 2)) {
      L.push(`  ${pad(cap(p.project, 25), 26)}${rpad(String(p.sessions), 4)}${rpad(String(p.turns), 7)}${rpad(fmt(p.usage.output), 7)}${rpad(fmt(p.usage.cacheCreate), 9)}${rpad(fmt(p.toolResultTokens), 9)}`);
    }
    if (r.byProject.length > opts.top * 2) L.push(`  … +${r.byProject.length - opts.top * 2} more`);
  }
  L.push('');
  ...
  for (const t of r.byTool.slice(0, opts.top * 2)) {
    L.push(`  ${pad(cap(t.name, 25), 26)}${rpad(String(t.calls), 6)}${rpad(fmt(t.resultTokens), 12)}${rpad(String(t.errors), 5)}`);
  }
  L.push('');

  L.push('by command head' + ' '.repeat(12) + 'calls  result-tok  err');
  for (const h of r.byHead.slice(0, opts.top * 2)) {
    L.push(`  ${pad(cap(h.key, 25), 26)}${rpad(String(h.calls), 6)}${rpad(fmt(h.resultTokens), 12)}${rpad(String(h.errors), 5)}`);
  }
  L.push('');
  ...
  for (const h of heaviestRows) {
    ...
    L.push(`  ${h.id}  ${pad(cap(h.project, 23), 24)}${rpad(String(h.turns), 5)} turns  ctx ~${rpad(fmt(h.avgContext), 5)}/turn  read ${rpad(fmt(h.cacheRead), 6)}  out ${rpad(fmt(h.output), 5)}  ${cross}`);
  }
  L.push('');
...
export function renderJson(r: Report, top: number, by?: 'session' | 'day', poolpool?: PoolpoolRow[]): string {
  const capped = { ...r, heaviest: by === 'session' ? r.heaviest : r.heaviest.slice(0, top) };
...
export function renderBrief(r: Report): string {
  const L: string[] = [];
  L.push(`tally — ${windowLabel(r)} · out ${fmt(r.usage.output)} · cache-read ${fmt(r.usage.cacheRead)} · ${r.sessions} sessions`);
...
export function renderMd(r: Report, opts: { top: number; by?: 'session' | 'day'; poolpool?: PoolpoolRow[] }): string {
  const L: string[] = [];
  const perTurn = r.turns ? Math.round(r.usage.cacheRead / r.turns) : 0;
  L.push(`# tally — ${windowLabel(r)} window`);
  L.push('');
  L.push(`${r.files} transcripts · ${r.sessions} sessions · ${r.turns.toLocaleString()} API turns · out **${fmt(r.usage.output)}** · cache-write **${fmt(r.usage.cacheCreate)}** · cache-read **${fmt(r.usage.cacheRead)}** · avg ctx/turn **~${fmt(perTurn)}**`);
  ...
  L.push('');
  L.push(`| session | project | turns | avg ctx | peak | >${fmt(r.ctxLimit)}@turn | burned >${fmt(r.ctxLimit)} |`);
  L.push('|---|---|--:|--:|--:|:-:|--:|');
  for (const h of heaviestRows) {
    L.push(`| ${h.id} | ${h.project} | ${h.turns} | ${fmt(h.avgContext)} | ${fmt(h.peakCtx)} | ${h.crossLimit ? 't' + h.crossLimit.turn : '—'} | ${h.burnedAbove ? '~' + fmt(h.burnedAbove) : '0'} |`);
  }
  L.push('');
  ...
  if (opts.by === 'day') {
    ...
    L.push('## By project');
    L.push('');
    L.push('| project | sess | turns | out | cache-w | tool-out |');
    L.push('|---|--:|--:|--:|--:|--:|');
    for (const p of r.byProject.slice(0, opts.top * 2)) {
      L.push(`| ${p.project} | ${p.sessions} | ${p.turns} | ${fmt(p.usage.output)} | ${fmt(p.usage.cacheCreate)} | ${fmt(p.toolResultTokens)} |`);
    }
    if (r.byProject.length > opts.top * 2) L.push(`| _… +${r.byProject.length - opts.top * 2} more_ | | | | | |`);
  }
  L.push('');
