...
/** ctx dropping below half its running peak = a compaction (same 0.5 as files.ts residentTurns);
 *  peak resets after each so consecutive loops all count. */
export function compactionEvents(s: Session): { turn: number; peakBefore: number; after: number }[] {
  const events: { turn: number; peakBefore: number; after: number }[] = [];
  let peak = 0;
  for (let i = 0; i < s.ctx.length; i++) {
    const cacheRead = s.ctx[i].cacheRead;
    if (peak > 0 && cacheRead < peak * 0.5) {
      events.push({ turn: i + 1, peakBefore: peak, after: cacheRead });
      peak = cacheRead;
    } else if (cacheRead > peak) {
      peak = cacheRead;
    }
  }
  return events;
...
/** Drops heredoc bodies (`<<'EOF' … EOF`) whose consuming command doesn't satisfy
 *  `isShellConsumer` (default: none do — always strip) — shared by `stripNonCodeSpans`
 *  (always strips) and `stripForCurlShCheck` (keeps a body a shell would actually execute). */
export function stripHeredocBodies(cmd: string, isShellConsumer: (headWord: string) => boolean = () => false): string {
  let out = '';
  let i = 0;
  const heredocStart = /<<-?\s*(['"]?)([A-Za-z_][A-Za-z0-9_]*)\1/g;
  let m: RegExpExecArray | null;
  while (i <= cmd.length && (heredocStart.lastIndex = i, (m = heredocStart.exec(cmd)))) {
    out += cmd.slice(i, heredocStart.lastIndex);
    const delim = m[2];
    const termRe = new RegExp(`^[ \\t]*${escapeRegExp(delim)}[ \\t]*$`, 'm');
    const rest = cmd.slice(heredocStart.lastIndex);
    const termMatch = termRe.exec(rest);
    const bodyEnd = termMatch ? heredocStart.lastIndex + termMatch.index + termMatch[0].length : cmd.length;
    const segStart = Math.max(cmd.lastIndexOf(';', m.index), cmd.lastIndexOf('&', m.index), cmd.lastIndexOf('|', m.index), cmd.lastIndexOf('\n', m.index)) + 1;
    const segment = cmd.slice(segStart, m.index).trim().split(/\s+/);
    const head = segment[0] === 'sudo' ? (segment[1] || '') : (segment[0] || '');
    if (isShellConsumer(head)) out += cmd.slice(heredocStart.lastIndex, bodyEnd);
    i = bodyEnd;
  }
  out += cmd.slice(i);
  return out;
...
const SHELL_HEAD_RE = /^(sh|bash|zsh|dash|ksh|eval)$/;

/** curl|sh check needs shell-consumed heredoc bodies to stay visible — an installer one-liner
 *  fed to `sh <<EOF` really executes, unlike one merely quoted or written to a file. */
export function stripForCurlShCheck(cmd: string): string {
  return stripQuotedSpans(stripHeredocBodies(cmd, (head) => SHELL_HEAD_RE.test(head)), false);
}

/** Drops single/double-quoted string contents (after heredoc bodies are gone) before the
 ...
 *
 *  `keepDoubleQuoted`: pre-bash's zsh-colon-modifier guard needs the opposite tradeoff for
 *  double quotes specifically — verified empirically that `"$ref:t"` still triggers zsh's `:t`
 *  modifier (quoting does NOT neutralize it, only bracing/`${ref}` does), so stripping
 *  double-quoted spans there would hide real positives. Single-quoted spans and heredoc bodies
 *  stay stripped either way (genuinely inert / accepted false negative, same as the default). */
export function stripNonCodeSpans(cmd: string, keepDoubleQuoted = false): string {
  return stripQuotedSpans(stripHeredocBodies(cmd), keepDoubleQuoted);
}

function stripQuotedSpans(heredocFree: string, keepDoubleQuoted: boolean): string {
  let result = '';
  let quote: '"' | "'" | null = null;
...
export function analyze(scan: Scan, since: number, until: number, top = 5, ctxLimit = DEFAULT_CTX_LIMIT, opts: AnalyzeOpts = {}): Report {
  ...
  // compact-loop: repeated compactions in one session — a different failure shape than
  // long-context (thrash, not overrun). Unshifts BEFORE long-context's own unshift below, so
  // long-context's unshift puts itself back in front: final order long-context, compact-loop, …
  const sumPeak = (ev: { peakBefore: number }[]) => ev.reduce((n, e) => n + e.peakBefore, 0);
  const compactLoops = [...scan.sessions.values()]
    .map((s) => ({ s, ev: compactionEvents(s) }))
    .filter((x) => x.ev.length >= 3 && x.s.ctx.length / x.ev.length <= 40)
    .sort((a, b) => sumPeak(b.ev) - sumPeak(a.ev));
  if (compactLoops.length) {
    findings.unshift({
      key: 'compact-loop',
      title: 'Compact loop (≥3 compactions in one session)',
      count: compactLoops.length,
      tokens: compactLoops.reduce((n, x) => n + sumPeak(x.ev), 0),
      hint: 'compact loops rebuild context just to re-compact it — raise/remove the compaction cap (autoCompactWindow) or split the task across sessions',
      samples: compactLoops.slice(0, top).map((x) => {
        const avgPeak = Math.round(sumPeak(x.ev) / x.ev.length);
        const avgAfter = Math.round(x.ev.reduce((n, e) => n + e.after, 0) / x.ev.length);
        return `${x.s.id.slice(0, 8)} ${x.s.project}: ${x.ev.length} compactions in ${x.s.ctx.length} turns, compacts near ~${fmt(avgPeak)}, lands at ~${fmt(avgAfter)}`;
      }),
    });
  }
  ...
  return {
    ...
    until,
    files: scan.files,
    sessionCount: scan.sessions.size,
