...
export async function loadSnap(name: string): Promise<Report | undefined> {
  try {
    const r = JSON.parse(await readFile(snapPath(name), 'utf8')) as Report & { sessions?: number };
    if (r.sessionCount === undefined && typeof r.sessions === 'number') r.sessionCount = r.sessions;
    return r;
  } catch (e) {
