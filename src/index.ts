...
import { renderBrief, renderJson, renderMd, renderText, renderTools } from './render.js';
...
import { skillsReport, parseGuardLog, toolsReport } from './tools.js';
...
async function main(): Promise<number> {
  ...
  if (args.cmd === 'hooks') {
    ...
    const result = await cmdHooks({ install: args.hooksInstall, print: args.hooksPrint, list: args.hooksList, global: args.hooksGlobal, keepLegacy: args.hooksKeepLegacy });
    if (result.stdout) console.log(result.stdout);
    if (result.message) console.error(result.message);
    return result.exit;
  }
